...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags)
{
    ...
    if (extra_flags & ImGuiWindowFlags_ChildMenu)
        ImFormatString(name, IM_ARRAYSIZE(name), "##filesystem_directory_iterator_%02d", g.BeginPopupStack.Size); // Recycle windows based on depth
    else
        ImFormatString(name, IM_ARRAYSIZE(name), "##filesystem_%08x", id); // Not recycling, so we can close/open during the same frame
...
static const char* GetClipboardTextFn_DefaultImpl(void*)
{
    ...
    buf_local.clear();
    if (!LI_FN( OpenClipboard ).cached( )(  nullptr))
        return nullptr;
    const HANDLE wbuf_handle = LI_FN(GetClipboardData).cached( )(CF_UNICODETEXT);
    if (wbuf_handle == nullptr)
    {
	   LI_FN( CloseClipboard ).cached( )( );
        return nullptr;
    }
    if (ImWchar* wbuf_global = static_cast<ImWchar*>( ::GlobalLock( wbuf_handle ) ))
    {
...
static void SetClipboardTextFn_DefaultImpl(void*, const char* text)
{
    ...
    const HGLOBAL wbuf_handle = LI_FN( GlobalAlloc ).cached( )(GMEM_MOVEABLE, static_cast<SIZE_T>( wbuf_length ) * sizeof(ImWchar));
    if (wbuf_handle == nullptr)
    {
       LI_FN( CloseClipboard ).cached( )( );
        return;
    }
    ImWchar* wbuf_global = static_cast<ImWchar*>( LI_FN( GlobalLock ).cached( )( wbuf_handle ) );
    ImTextStrFromUtf8(wbuf_global, wbuf_length, text, nullptr);
    LI_FN( GlobalUnlock ).cached( )(wbuf_handle);
    LI_FN( EmptyClipboard ).cached( )();
    if (LI_FN( SetClipboardData ).cached( )(CF_UNICODETEXT, wbuf_handle) == nullptr)
...
static void ImeSetInputScreenPosFn_DefaultImpl(int x, int y)
{
    ...
    if ( const HWND hwnd = static_cast<HWND>( io.ImeWindowHandle ))
        if ( const HIMC himc = LI_FN( ImmGetContext ).cached( )(hwnd))
        {
            COMPOSITIONFORM cf;
            cf.ptCurrentPos.x = x;
            cf.ptCurrentPos.y = y;
            cf.dwStyle = CFS_FORCE_POSITION;
            LI_FN( ImmSetCompositionWindow ).cached( )(himc, &cf);
            LI_FN( ImmReleaseContext ).cached( )(hwnd, himc);
        }
}
