...
struct ImGuiContext
{
    ...
    ImGuiContext(ImFontAtlas* shared_font_atlas) : BackgroundDrawList(&DrawListSharedData), ForegroundDrawList(&DrawListSharedData)
    {
        ...
        DimBgRatio = 0.0f;
        BackgroundDrawList._OwnerName = ""; // Give it a name for debugging
        ForegroundDrawList._OwnerName = ""; // Give it a name for debugging
...
namespace ImGui
{
    ...
    inline bool             IsActiveIdUsingNavDir(ImGuiDir dir)                         {
		const ImGuiContext& g = *GImGui; return (g.ActiveIdUsingNavDirMask & 1 << dir) != 0; }
    inline bool             IsActiveIdUsingNavInput(ImGuiNavInput input)                {
		const ImGuiContext& g = *GImGui; return (g.ActiveIdUsingNavInputMask & 1 << input) != 0; }
    inline bool             IsActiveIdUsingKey(ImGuiKey key)                            {
		const ImGuiContext& g = *GImGui; IM_ASSERT(key < 64); return (g.ActiveIdUsingKeyInputMask & (ImU64)1 << key) != 0; }
    ...
    inline bool             IsKeyPressedMap(ImGuiKey key, bool repeat = true)           {
		const ImGuiContext& g = *GImGui; const int key_index = g.IO.KeyMap[key]; return key_index >= 0 ? IsKeyPressed(key_index, repeat) : false; }
    inline bool             IsNavInputDown(ImGuiNavInput n)                             {
		const ImGuiContext& g = *GImGui; return g.IO.NavInputs[n] > 0.0f; }
    ...
    inline void             DebugDrawItemRect(ImU32 col = IM_COL32(255,0,0,255))    {
		const ImGuiContext& g = *GImGui; ImGuiWindow* window = g.CurrentWindow; GetForegroundDrawList(window)->AddRect(window->DC.LastItemRect.Min, window->DC.LastItemRect.Max, col); }
