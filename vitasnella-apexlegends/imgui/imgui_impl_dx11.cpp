...
void ImGui_ImplDX11_RenderDrawLists(ImDrawData* draw_data)
{
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++)
	{
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
		{
			...
			{
				const D3D11_RECT r = { static_cast<LONG>( pcmd->ClipRect.x ), static_cast<LONG>( pcmd->ClipRect.y ), static_cast<LONG>( pcmd->ClipRect.z ), static_cast<LONG>( pcmd->ClipRect.w ) };
...
static void ImGui_ImplDX11_CreateFontsTexture()
{
	...
	io.Fonts->TexID = static_cast<void*>( g_pFontTextureView );
...
bool ImGui_ImplDX11_CreateDeviceObjects()
{
	...
	// Create the vertex shader
	{
		const std::string vertexShader =
			_( "cbuffer vertexBuffer : register(b0) \
            {\
		...
		LI_FN( D3DCompile ).cached( )(vertexShader.c_str( ), vertexShader.length( ), nullptr, nullptr, nullptr, "main", "vs_4_0", 0, 0, &g_pVertexShaderBlob, nullptr);
		...
		const D3D11_INPUT_ELEMENT_DESC local_layout[] = {
			{ "POSITION", 0, DXGI_FORMAT_R32G32_FLOAT,   0, (size_t)(&static_cast<ImDrawVert*>( nullptr )->pos), D3D11_INPUT_PER_VERTEX_DATA, 0 },
		{ "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT,   0, (size_t)(&static_cast<ImDrawVert*>( nullptr )->uv),  D3D11_INPUT_PER_VERTEX_DATA, 0 },
		{ "COLOR",    0, DXGI_FORMAT_R8G8B8A8_UNORM, 0, (size_t)(&static_cast<ImDrawVert*>( nullptr )->col), D3D11_INPUT_PER_VERTEX_DATA, 0 },
		};
	...
	// Create the pixel shader
	{
		...
		LI_FN( D3DCompile ).cached( )(pixelShader.c_str( ), pixelShader.length(  ), nullptr, nullptr, nullptr, _( "main" ), _( "ps_4_0" ), 0, 0, &g_pPixelShaderBlob, nullptr);
...
bool    ImGui_ImplDX11_Init(void* hwnd, ID3D11Device* device, ID3D11DeviceContext* device_context)
{
	...
	if (!LI_FN( QueryPerformanceFrequency ).cached( )((LARGE_INTEGER *)&g_TicksPerSecond))
		return false;
	if (!LI_FN( QueryPerformanceCounter ).cached( )((LARGE_INTEGER *)&g_Time))
		return false;
...
void ImGui_ImplDX11_NewFrame()
{
	...
	// Setup time step
	INT64 current_time;
	LI_FN( QueryPerformanceCounter ).cached( )((LARGE_INTEGER *)&current_time);
	...
	if (io.MouseDrawCursor)
		LI_FN( SetCursor ).cached( )( nullptr);
