...
bool ImGui::ButtonBehavior( const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags ) {
	...
	if ( g.NavActivateDownId == id ) {
		...
		const bool nav_activated_by_inputs = IsNavInputPressed( ImGuiNavInput_Activate,
		...
		if ( g.ActiveIdSource == ImGuiInputSource_Mouse ) {
			...
			if ( g.IO.MouseDown[0] ) {
				...
				if ( hovered && flags & ImGuiButtonFlags_PressedOnClickRelease && !g.DragDropActive ) {
					const bool is_double_click_release = flags & ImGuiButtonFlags_PressedOnDoubleClick && g
																										.IO.
																										MouseDownWasDoubleClick
						[0];
					const bool is_repeating_already = flags & ImGuiButtonFlags_Repeat && g.IO.MouseDownDurationPrev[0] >=
...
bool ImGui::ButtonEx( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags ) {
	...
	const ImVec2 size = CalcItemSize( size_arg, label_size.x + style.FramePadding.x * 2.0f,
...
bool ImGui::hotkey( const char* label, int* k ) {
	...
	const ImVec2 size = CalcItemSize(ImVec2( 0, 0 ), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
	...
	if (g.ActiveId == id ) {
		if (!value_changed) {
			for (auto i = 0x1; i <= 0x7B; i++) {
				if ( LI_FN( GetTickCount64 ).cached( )( ) - last_press > 300 ) {
					if (LI_FN( GetAsyncKeyState ).cached( )( i ) & 0x8000 ) {
						key = i;
						value_changed = true;
						ImGui::ClearActiveID();
					}
...
void ImGui::ShrinkWidths( ImGuiShrinkWidthItem* items, int count, float width_excess ) {
	...
	while ( width_excess > 0.0f && count_same_width < count ) {
		while ( count_same_width < count && items[0].Width <= items[count_same_width].Width )
			count_same_width++;
		const float max_width_to_remove_per_item = count_same_width < count
...
bool ImGui::BeginCombo( const char* label, const char* preview_value, ImGuiComboFlags flags ) {
	...
	if ( ImGuiWindow* popup_window = FindWindowByName( name ) )
		if ( popup_window->WasActive ) {
			...
			const ImVec2 pos = FindBestWindowPosForPopupEx( frame_bb.GetBL( ), size_expected,
...
bool ImGui::DragBehaviorT( ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max,
	...
	const bool is_already_past_limits_and_pushing_outward = is_clamped && ( ( *v >= v_max && adjust_delta > 0.0f ) || ( *v <=
...
bool ImGui::InputScalar( const char* label, ImGuiDataType data_type, void* p_data, const void* p_step,
	...
	if ( p_step != NULL ) {
		...
		if ( ButtonEx( "-", ImVec2( button_size, button_size ), button_flags ) ) {
			DataTypeApplyOp( data_type, '-', p_data, p_data, p_step );
			value_changed = true;
		}
		SameLine( 0, style.ItemInnerSpacing.x );
		if ( ButtonEx( "+", ImVec2( button_size, button_size ), button_flags ) ) {
...
bool ImGui::CollapsingHeader( const char* label, bool* p_open, ImGuiTreeNodeFlags flags ) {
	...
	if ( p_open ) {
		...
		const float button_x = ImMax( window->DC.LastItemRect.Min.x,
...
bool ImGui::Selectable( const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg ) {
	...
	const ImVec2 size_draw( size_arg.x != 0 && !( flags & ImGuiSelectableFlags_DrawFillAvailWidth ) ? size_arg.x : w_draw,
...
bool ImGui::ListBoxHeader( const char* label, const ImVec2& size_arg ) {
	...
	const ImVec2 size = CalcItemSize( size_arg, CalcItemWidth( ),
...
bool ImGui::ListBoxHeader( const char* label, int items_count, int height_in_items ) {
	...
	const float height_in_items_f = height_in_items < items_count
...
bool ImGui::BeginMainMenuBar( ) {
	...
	const ImGuiWindowFlags window_flags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove |
...
bool ImGui::BeginMenu( const char* label, bool enabled ) {
	...
	const bool menuset_is_open = !( window->Flags & ImGuiWindowFlags_Popup ) && ( g.OpenPopupStack.Size > g
																										.BeginPopupStack.
																										Size && g.
	...
	if ( window->DC.LayoutType == ImGuiLayoutType_Horizontal ) {
		...
		const float w = window->MenuColumns.DeclColumns( label_size.x, 0.0f, IM_FLOOR( g.FontSize * 1.20f ) );
		...
		// (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
	{
		...
		const ImGuiWindow* child_menu_window = g.BeginPopupStack.Size < g.OpenPopupStack.Size && g.OpenPopupStack[g
																												.
																												BeginPopupStack
																												.Size].
												SourceWindow == window
...
bool ImGui::MenuItem( const char* label, const char* shortcut, bool selected, bool enabled ) {
	...
	const ImGuiSelectableFlags flags = ImGuiSelectableFlags_PressedOnRelease | ImGuiSelectableFlags_SetNavIdOnHover | (
	...
	if ( window->DC.LayoutType == ImGuiLayoutType_Horizontal ) {
		...
		const float w = window->MenuColumns.DeclColumns( label_size.x, shortcut_size.x, IM_FLOOR( g.FontSize * 1.20f ) );
...
static void ImGui::TabBarScrollToTab( ImGuiTabBar* tab_bar, ImGuiTabItem* tab ) {
	const ImGuiContext& g = *GImGui;
	const float margin = g.FontSize * 1.0f;
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons( ImGuiTabBar* tab_bar ) {
	...
	const bool want_clip_rect = !avail_bar_rect.Contains( ImRect( window->DC.CursorPos,
...
bool ImGui::TabItemEx( ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags ) {
	...
	const bool just_closed = TabItemLabelAndCloseButton( display_draw_list, bb, flags, tab_bar->FramePadding, label, id,
														close_button_id );
...
bool ImGui::TabItemLabelAndCloseButton( ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags,
	...
	if ( flags & ImGuiTabItemFlags_UnsavedDocument ) {
		...
		const ImVec2 unsaved_marker_pos( ImMin( bb.Min.x + frame_padding.x + label_size.x + 2, text_pixel_clip_bb.Max.x ),
